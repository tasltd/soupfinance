...
test.describe('Settings - API Logging', () => {
  test.beforeEach(async ({ page }) => {
    await setupAuth(page);
  });

  test('API requests include X-Api-Consumer header', async ({ page }) => {
    // This test only makes sense in mock mode - we can verify headers on mocked routes
    if (isLxcMode()) {
      test.skip();
      return;
    }
    ...
    await page.route('**/rest/agent/index.json*', (route) => {
      ...
      capturedHeader = headers['x-api-consumer'] || null;
    ...
    await page.goto('/settings/users');
    await page.waitForSelector('[data-testid="user-list-page"]');

    // Verify header was sent
    expect(capturedHeader).toBe('SOUPFINANCE');
  });
});
