...
Use `backendTestUsers` from `e2e/fixtures.ts` for credentials. **768 unit tests** across 26 test files. **361 mock E2E tests** (15 spec files). **159/170 integration tests** pass against LXC backend (6 failed: backend Hibernate proxy bug in vendor CRUD, 5 skipped). See `e2e/integration/INTEGRATION-TEST-RESULTS.md` for detailed results.
...
- Dark mode: `dark:` prefix classes (toggled via `html.dark` class, state in `uiStore`). **CRITICAL**: Tailwind v4 requires `@custom-variant dark (&:where(.dark, .dark *));` in `src/index.css` after `@import "tailwindcss";` to enable class-based dark mode — without this, `dark:` utilities only respond to `prefers-color-scheme` media query (OS setting), not the `.dark` class. Uses **paired semantic tokens**: `bg-surface-light dark:bg-surface-dark`, `text-text-light dark:text-text-dark`, `border-border-light dark:border-border-dark`. Every element needs both light AND dark classes. Do NOT use non-existent shorthand tokens like `bg-surface` or `text-primary-text`
